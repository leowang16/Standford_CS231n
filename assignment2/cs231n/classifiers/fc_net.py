...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dz1, dW2, db2 = affine_backward(da2, cache2)
        dz0, dW1, db1 = affine_relu_backward(dz1, cache1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for i in xrange(self.num_layers):
            # First layer
            if i == 0:
                self.params["W%d" % (i + 1)] = np.random.normal(
                    0, weight_scale, [input_dim, hidden_dims[0]])
                self.params["b%d" % (i + 1)] = np.zeros(hidden_dims[0])
                if self.use_batchnorm:
                    self.params["gamma%d" %
                                (i + 1)] = np.ones([1, hidden_dims[0]])
                    self.params["beta%d" %
                                (i + 1)] = np.zeros([1, hidden_dims[0]])
            # Last layer
            ...
            # Hidden layers
                ...
                self.params["W%d" % (i + 1)] = np.random.normal(
                    0, weight_scale, [hidden_dims[i - 1], hidden_dims[i]])
                self.params["b%d" % (i + 1)] = np.zeros(hidden_dims[i])
                if self.use_batchnorm:
                    self.params["gamma%d" %
                                (i + 1)] = np.ones([1, hidden_dims[i]])
                    self.params["beta%d" %
                                (i + 1)] = np.zeros([1, hidden_dims[i]])
        #######################################################################
        #                             END OF YOUR CODE                             #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in xrange(self.num_layers):
            ...
            if i != self.num_layers - 1:
                if self.use_dropout:
                    z, cache = affine_relu_dropout_forward(
                        z, W, b, self.dropout_param)
                elif self.use_batchnorm:
                    gamma, beta = self.params["gamma%d" % (
                        i + 1)], self.params["beta%d" % (i + 1)]
                    z, cache = affine_batchnorm_relu_forward(
                        z, W, b, gamma, beta, self.bn_params[i])
                else:
        ...
        for i in range(self.num_layers, 0, -1):
            ...
            if i == self.num_layers:
                ...
                for j in range(self.num_layers):
                    loss += 0.5 * self.reg * \
                        np.sum(np.square(self.params["W%d" % (j + 1)]))
                dz, dW, db = affine_backward(dz, caches.pop())
            else:
                if self.use_dropout:
                    dz, dW, db = affine_relu_dropout_backward(dz, caches.pop())
                elif self.use_batchnorm:
                    dz, dW, db, dgamma, dbeta = affine_batchnorm_relu_backward(
                        dz, caches.pop())
