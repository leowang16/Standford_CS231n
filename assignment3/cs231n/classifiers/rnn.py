...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
            ...
            raise ValueError('cell_type is wrong')
        ...
        if self.cell_type == 'rnn':
            dh, dh0, dWx, dWh, db = rnn_backward(dh, cache_rnn)
        elif self.cell_type == 'lstm':
            dh, dh0, dWx, dWh, db = lstm_backward(dh, cache_lstm)
        else:
            raise ValueError('cell_type is wrong')
        
        dW_embed = word_embedding_backward(dh, cache_embedding)
    ...
    def sample(self, features, max_length=30):
        ...
        for t in xrange(max_length):
            # step 1
            word_embedding, _ = word_embedding_forward(x, W_embed)
            # step 2
            if self.cell_type == 'rnn':
                h, _ = rnn_step_forward(word_embedding, prev_h, Wx, Wh, b)
            elif self.cell_type == 'lstm':
                h, c, _ = lstm_step_forward(word_embedding, prev_h, prev_c, Wx, Wh, b)
            else:
                raise ValueError('cell_type is wrong')
            # step 3
